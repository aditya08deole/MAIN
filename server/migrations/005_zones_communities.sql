-- ============================================================================
-- MIGRATION 005: ZONES AND COMMUNITIES TABLES
-- ============================================================================
-- Purpose: Add hierarchical structure for zones and communities
-- Author: System Architect
-- Date: 2026-02-21
-- Requirements: Support for multi-zone community management
-- ============================================================================
...
CREATE INDEX IF NOT EXISTS idx_regions_name ON zones(name);
CREATE INDEX IF NOT EXISTS idx_communities_region_id ON communities(zone_id);
...
DROP TRIGGER IF EXISTS regions_updated_at_trigger ON zones;
...
-- Step 9: Seed major Indian cities as zones
INSERT INTO zones (name, state) VALUES
    ('Mumbai', 'Maharashtra'),
    ('Delhi', 'Delhi'),
    ('Bangalore', 'Karnataka'),
    ('Hyderabad', 'Telangana'),
...
COMMENT ON TABLE zones IS 'Geographic zones (cities) for organizing communities';
COMMENT ON TABLE communities IS 'Communities within zones where devices are deployed';
...
DECLARE
    ...
    index_count INTEGER;
BEGIN
    SELECT COUNT(*) INTO region_count FROM zones;
    SELECT COUNT(*) INTO index_count 
    FROM pg_indexes 
    WHERE tablename IN ('zones', 'communities');
    
    RAISE NOTICE 'Migration 005 Complete:';
    RAISE NOTICE '  - Total zones: %', region_count;
