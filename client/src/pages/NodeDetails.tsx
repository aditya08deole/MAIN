...
import EvaraTankAnalytics from './EvaraTankAnalytics';
import EvaraDeepAnalytics from './EvaraDeepAnalytics';
import EvaraFlowAnalytics from './EvaraFlowAnalytics';
...
const NodeDetails = () => {
    ...
    if (device) {
        ...
        const isOnline = device.status === 'Online' || device.status === 'Working' || device.status === 'Running';

        return (
            <div className="flex flex-col min-h-full apple-glass-inner">
                <div className="p-6 max-w-4xl mx-auto w-full">
                    <button onClick={() => navigate(-1)} className="flex items-center gap-2 text-slate-600 hover:text-slate-900 mb-6 transition-colors">
                        <ArrowLeft size={20} />
                    ...
                    <div className={clsx("bg-gradient-to-br rounded-3xl p-8 border border-slate-200 shadow-xl", styles.accentBg)}>
                        <div className="flex items-start gap-4 mb-6">
                            ...
                            <div className="flex-1">
                                ...
                                <div className="flex gap-2 flex-wrap">
                                    <span className={clsx("px-3 py-1 rounded-full text-sm font-bold", styles.badge)}>{label}</span>
                                    <span className={clsx("px-3 py-1 rounded-full text-sm font-bold", isOnline ? "bg-green-100 text-green-700" : "bg-red-100 text-red-700")}>{device.status}</span>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            <div className="apple-glass-card backdrop-blur rounded-xl p-4">
                                <p className="text-xs font-bold text-slate-500 uppercase mb-1">Blueprint ID</p>
                ...
                <div className="p-6">
                    {device.asset_type === 'tank' && <EvaraTankAnalytics />}
                    {device.asset_type === 'bore' && <EvaraDeepAnalytics />}
                    {device.asset_type === 'pump' && <EvaraFlowAnalytics />}
                </div>
            </div>
        );
    }
    ...
    if (!device && !node) {
        return (
            <div className="flex flex-col items-center justify-center h-full py-32 text-center apple-glass-inner">
                ...
                <h2 className="text-lg font-bold text-slate-600 mb-1">Asset Not Found</h2>
                <p className="text-sm text-slate-400 mb-6">Blueprint ID: {id}</p>
                <button onClick={() => navigate(-1)} className="px-5 py-2.5 bg-blue-600 text-white rounded-xl text-sm font-semibold hover:bg-blue-700 transition-colors">Go Back</button>
            </div>
        );
    }
