...
export const communitySchema = z.object({
    ...
    zone_id: z.string().uuid('Please select a valid zone'),
    address: z.string().optional(),
    pincode: z.string().regex(/^\d{6}$/, 'Invalid pincode (6 digits required)').optional().or(z.literal('')),
    contact_person: z.string().optional(),
    contact_email: z.string().email('Invalid email').optional().or(z.literal('')),
    contact_phone: z.string().optional(),
    operational_status: z.enum(['active', 'pending', 'inactive']).default('active'),
});
...
export const customerSchema = z.object({
    ...
    community_id: z.string().uuid('Please select a valid community'),
