...
export default function RealisticTank({
    ...
    return (
        <div style={{ position: 'relative', width: 130, height: 280, filter: 'drop-shadow(0 10px 20px rgba(0,0,0,0.15))' }}>
            <svg
                viewBox="0 0 130 280"
            ...
            >
                <defs>
                    ...
                    <clipPath id={`tankClip_${uid}`}>
                        <rect x={TANK_X} y={TANK_Y} width={TANK_WIDTH} height={TANK_HEIGHT} rx={rx} ry={rx} />
                    </clipPath>

                    {/* Wave Animation */}
                    <pattern id={`wavePattern_${uid}`} x="0" y="0" width="200" height="20" patternUnits="userSpaceOnUse">
                        <path d="M 0 10 Q 25 0 50 10 T 100 10 T 150 10 T 200 10 V 20 H 0 Z" fill={waterColor.wave} opacity="0.4">
                            <animateTransform attributeName="transform" type="translate" from="0 0" to="-100 0" dur="3s" repeatCount="indefinite" />
                        </path>
                        <path d="M 0 12 Q 25 2 50 12 T 100 12 T 150 12 T 200 12 V 22 H 0 Z" fill={waterColor.wave} opacity="0.2">
                            <animateTransform attributeName="transform" type="translate" from="-50 0" to="50 0" dur="4s" repeatCount="indefinite" />
                        </path>
                    </pattern>
                </defs>

                {/* ── Metal Caps ── */}
                <rect x={TANK_X + 25} y={10} width={50} height={12} rx={4} fill="#94a3b8" />
                <rect x={TANK_X + 20} y={TANK_Y + TANK_HEIGHT - 2} width={60} height={12} rx={4} fill="#475569" />
                        ...
                        <g opacity="0.3">
                            {[1, 2, 3, 4, 5].map(i => (
                                <circle key={i} r={1.5 + Math.random() * 2} fill="white">
                                    <animate attributeName="cx" from={TANK_X + 20 + Math.random() * 60} to={TANK_X + 20 + Math.random() * 60} dur={`${3 + Math.random() * 2}s`} repeatCount="indefinite" />
                                    <animate attributeName="cy" from={TANK_Y + TANK_HEIGHT} to={waterY} dur={`${4 + Math.random() * 4}s`} begin={`${i * 0.8}s`} repeatCount="indefinite" />
                                    <animate attributeName="opacity" values="0;0.5;0" dur={`${4 + Math.random() * 4}s`} begin={`${i * 0.8}s`} repeatCount="indefinite" />
                                </circle>
                            ))}
                        </g>
                    )}
                ...
                {[20, 40, 60, 80].map(t => (
                    <line
                        key={t}
                        x1={TANK_X + TANK_WIDTH - 10} y1={TANK_Y + TANK_HEIGHT - (t / 100) * TANK_HEIGHT}
                        x2={TANK_X + TANK_WIDTH} y2={TANK_Y + TANK_HEIGHT - (t / 100) * TANK_HEIGHT}
                        stroke="rgba(255,255,255,0.2)"
                        strokeWidth="1"
                    />
                ))}
                ...
                {!loading && (
                    <g>
                        <text
                            ...
                            style={{ fontFamily: 'Inter, sans-serif', textShadow: fillPct > 50 ? '0 2px 4px rgba(0,0,0,0.3)' : 'none' }}
                        >
